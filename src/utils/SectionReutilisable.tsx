...
const SectionReutilisable: React.FC<sectionReutilisableProps> = ({ arr, sectionTitle,hrefShopNow }) => {
    ...
    return (
        <div className='z-10 relative flex justify-center flex-col items-center mt-8 md:mt-16 mx-5 md:mx-18'>
            ...
            <a className='text-sm underline text-gray-600' href={hrefShopNow}>Shop now</a>

            {sectionTitle === 'New arrival' && sliderButtons}

            <div ref={targetRef} className='flex items-center justify-evenly w-full gap-4 mt-8 flex-row overflow-x-scroll md:overflow-hidden md:flex-row'>
                {arr.map(({ id, title, price, images, link }, index) => (
                    <NewArrivalCard
