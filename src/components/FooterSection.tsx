                ...
                <nav className='pt-10 md:pt-30 pb-10'>
                    <ul className='flex md:justify-start justify-center flex-wrap'>
                        {footerLinks.map(({ id, title, link }) => (
                            <li className='hover:text-gray-300 transition duration-200 text-[12px]' key={id}>
                                &nbsp; <a href={link}>{title}</a> |
                            </li>
                        ))}
                    </ul>
                    <a className='pt-4 md:pt-0 flex justify-center items-center md:justify-start text-center md:text-left text-[12px]' href="/">
                        &nbsp; Copyright © {year} KBDfans® Mechanical Keyboards Store. Powered by Reutchi Dan
                    </a>
                </nav>
            </div>
        </footer>
